...
def process_frame(frame):
    ...
    bg_label = labels[0, 0]
    bg_mask = (labels == bg_label) | is_red

    # 5. Dilate mask slightly to cleanly encompass edge transition pixels
    bg_mask_dilated = cv2.dilate(bg_mask.astype(np.uint8), KERNEL, iterations=1)
    bg_mask_smooth = cv2.GaussianBlur(bg_mask_dilated.astype(np.float32), (5, 5), 1.2)[:, :, np.newaxis]

    # 6. Despill: Neutralize any red bounce/fringe along the foreground edge
    fg = resized.copy()
    fg_b, fg_g, fg_r = cv2.split(fg)
    spill_mask = (fg_r.astype(int) > fg_g.astype(int) + 30) & (bg_mask_smooth[:, :, 0] < 0.95) & (bg_mask_smooth[:, :, 0] > 0.05)
